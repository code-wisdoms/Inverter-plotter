...
module.exports = {
    ...
    createTable(cback) {
        ...
        `, (res, err) => {
            ...
            db.run(`CREATE TABLE if not exists flags (
                ...
                changed INTEGER NOT NULL);`, (res, err) => {
                err && console.log(err);
                cback && cback();
            });
        });
    ...
    updateFlag(name, value) {
        db.run(`UPDATE flags SET value = ?, changed = ${new Date().getTime()} WHERE name = '${name}'`, [value]);
    },
    addFlag(name, value) {
        this.select(`SELECT ${name} {t} ORDER BY dated DESC LIMIT 1`, (err, rows) => {
            err && console.log(err);
            !err && db.run(`INSERT INTO flags(name,value,variation,changed)VALUES (?,?,?,?);`, [name, rows.length ? rows[0][name] : 0, value, new Date().getTime()], (res, err) => {
                err && console.log(err);
            });
        });
    },
